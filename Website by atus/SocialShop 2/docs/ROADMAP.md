...
## ✅ Package 2 — Backend thật: Auth + Database (đang giao)
- Kết nối MongoDB thật (`server/config/db.js`, Mongoose).
- Model `User` (hash password bằng bcrypt, ẩn password khi trả JSON).
- API: `POST /api/auth/register`, `POST /api/auth/login`, `GET /api/auth/me`.
- JWT: cấp token khi đăng ký/đăng nhập, middleware `requireAuth` bảo vệ route.
- Middleware xử lý lỗi tập trung (`error.middleware.js`).
- Trang **Đăng nhập** (`client/pages/login.html`) và **Đăng ký**
  (`client/pages/register.html`), gọi API thật qua `client/js/auth.js`.
- Top bar ở Feed/Marketplace tự đổi icon 👤 → 🟢 khi đã đăng nhập, bấm
  vào để đăng xuất.
- File `server/.env.example` — copy thành `.env` và điền `MONGO_URI`,
  `JWT_SECRET` của bạn (dùng MongoDB Atlas free tier là đủ).
